...
bool MainObject::LoadImg(std::string path, SDL_Renderer* screen) //show character
{
...
void MainObject::set_clips() //set frame
{
...
void MainObject::CenterEntityOnMap(Map& map_data) //screen move with character
{
...
void MainObject::CheckToMap(Map& map_data){
    ...
    {
        if (x_val_ > 0) //character is moving to the right side
        {
        ...
        else if (x_val_ < 0) //character is moving to the left side
        {
    ...
    if (x1 >= 0 && x2 <= MAX_MAP_X && y1 >= 0 && y2 < MAX_MAP_Y)
    {
        if (y_val_ > 0) //character is moving down
        {
        ...
        else if (y_val_ < 0) //character is moving up (jump)
        {
...
void MainObject::UpdateImagePlayer(SDL_Renderer* des) {
    ...
    else if (on_ground_ == false)
    {
        if (status_ == WALK_LEFT)
        {
            LoadImg("img/player_jump_left.png", des);
        }
        else if (status_ == WALK_RIGHT)
        {
            LoadImg("img/player_jump_right.png", des);
        }
        else
        {
            LoadImg("img/player_idle.png", des);
        }
    }
}
